...
window.exportAllData = async function() {
    try {
        if (typeof ChatBackup === 'undefined') {
            showNotification('备份引擎未加载，请刷新页面', 'error');
            return;
        }
        showNotification('正在打包全量备份（ZIP）…', 'info', 3000);
...
window.importAllData = async function(file) {
    ...
    try {
        if (typeof ChatBackup === 'undefined') {
            showNotification('备份引擎未加载', 'error');
            return;
        }
        showNotification('正在解析备份文件…', 'info', 2000);
        const data = await ChatBackup.loadBackupFromFile(file);
        await ChatBackup.applyBackupToStorage(data, { selective: false });
        showNotification('全量恢复成功，页面即将刷新', 'success', 2000);
        setTimeout(() => location.reload(), 1500);
    } catch (e) {
        console.error('[importAllData]', e);
        showNotification('导入失败：' + e.message, 'error');
    }
};
// ===== 数据管理抽屉控制（供 data.js 内联 onclick 调用） =====
// 改为全局函数，并增加容错
window.openDrawer = function(drawerId) {
    ...
    if (!drawer) {
        console.error('[openDrawer] 抽屉元素不存在:', drawerId);
        return;
    }
