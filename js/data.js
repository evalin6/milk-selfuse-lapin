(function () {
    ...
    function ensureDrawersOnBody() {
        ...
        DRAWER_IDS.forEach(function(id) {
            ...
            if (existing) {
                ...
                // 如果尚未绑定事件，则绑定（防止手动移动后丢失）
                if (!existing.dataset.eventsBound) {
                    existing.dataset.eventsBound = '1';
                    bindDrawerEvents(existing, id);
                }
                return;
            }
            ...
            var drawer = dummy.firstElementChild;   // 关键：先获取元素再追加
            document.body.appendChild(drawer);

            // 标记并绑定事件
            drawer.dataset.eventsBound = '1';
            bindDrawerEvents(drawer, id);
        });
    }

            // 把抽屉事件绑定提取为独立函数
    // 把抽屉事件绑定提取为独立函数
    function bindDrawerEvents(drawer, id) {
        if (id === 'dm-drawer-full') {
            var backdrop = drawer.querySelector('#dm-drawer-full-backdrop');
            if (backdrop) backdrop.addEventListener('click', function () { closeDrawer('dm-drawer-full'); });
            var cancelBtn = drawer.querySelector('#dm-drawer-full-cancel');
            if (cancelBtn) cancelBtn.addEventListener('click', function () { closeDrawer('dm-drawer-full'); });
            var exportBtn = drawer.querySelector('#export-all-settings-real');
            if (exportBtn) exportBtn.addEventListener('click', function () {
                closeDrawer('dm-drawer-full');
                if (typeof exportAllData === 'function') exportAllData();
            });
            var importBtn = drawer.querySelector('#import-all-settings-real');
        ...
        } else if (id === 'dm-drawer-chat') {
            var backdrop = drawer.querySelector('#dm-drawer-chat-backdrop');
            if (backdrop) backdrop.addEventListener('click', function () { closeDrawer('dm-drawer-chat'); });
            var cancelBtn = drawer.querySelector('#dm-drawer-chat-cancel');
            if (cancelBtn) cancelBtn.addEventListener('click', function () { closeDrawer('dm-drawer-chat'); });
            var exportBtn = drawer.querySelector('#export-chat-btn-real');
            if (exportBtn) exportBtn.addEventListener('click', function () {
                closeDrawer('dm-drawer-chat');
                if (typeof exportChatHistory === 'function') exportChatHistory();
            });
            var importBtn = drawer.querySelector('#import-chat-btn-real');
            if (importBtn) importBtn.addEventListener('click', function () {
                closeDrawer('dm-drawer-chat');
                var inp = document.createElement('input');
    ...
    function bindAll(mc) {
        ...
        if (tileChatBackup) tileChatBackup.addEventListener('click', function () { openDrawer('dm-drawer-chat'); });
            ...
            // 抽屉事件：如果已经通过 ensureDrawersOnBody 绑定过，这里不会重复绑定
        // 因为 addEventListener 可以重复绑定，但为了避免重复，可以检查标记
        var fullDrawer = document.getElementById('dm-drawer-full');
        if (fullDrawer && !fullDrawer.dataset.eventsBound) {
            fullDrawer.dataset.eventsBound = '1';
            bindDrawerEvents(fullDrawer, 'dm-drawer-full');
        }
        
        var chatDrawer = document.getElementById('dm-drawer-chat');
        if (chatDrawer && !chatDrawer.dataset.eventsBound) {
            chatDrawer.dataset.eventsBound = '1';
            bindDrawerEvents(chatDrawer, 'dm-drawer-chat');
        } 
