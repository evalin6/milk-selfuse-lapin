...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    if (settings.musicPlayerEnabled) {
        ...
        function initCoreListeners() {
            ...
            // 定义一个自动调整高度的函数（可放在 initCoreListeners 内部或外部）
        function autoResizeTextarea(textarea) {
            ...
            const minHeight = parseFloat(cs.minHeight) || 46; // 假设 min-height 是总高度（border-box）
            const MAX_LINES = 5.5;

            // 最大总高度 = 行数 * 行高 + 内边距 + 边框（因为 border-box 下总高度包含这些）
            const maxTotalHeight = MAX_LINES * lineHeight + paddingTop + paddingBottom + borderTop + borderBottom;
            // 当前内容的实际总高度 = scrollHeight（含 padding） + border（scrollHeight 不含 border）
            const currentTotalHeight = textarea.scrollHeight + borderTop + borderBottom;

            // 目标总高度：不低于最小高度，不超过最大高度
            let targetTotalHeight = Math.max(minHeight, Math.min(currentTotalHeight, maxTotalHeight));
            textarea.style.height = targetTotalHeight + 'px';

            console.log('lineHeight:', lineHeight, 'maxTotalHeight:', maxTotalHeight, 'scrollHeight:', textarea.scrollHeight, 'targetTotalHeight:', targetTotalHeight);
        }
